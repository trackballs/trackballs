...
void Teleport::draw() {
  ...
  glBegin(GL_TRIANGLE_FAN);
  {
    ...
    glVertex3f(0., 0., 0.3);
    for (i = 0; i <= 16; i++) {
      glNormal3f(cos(2. * 3.14159 * i / 16.), sin(2. * 3.14159 * i / 16.), .4);
      glVertex3f(radius * cos(2. * 3.14159 * i / 16.), radius * sin(2. * 3.14159 * i / 16.),
                 0.1);
    }
  }
  glEnd();
...
void Teleport::draw2() {
  ...
  glColor4f(secondaryColor[0], secondaryColor[1], secondaryColor[2],
            secondaryColor[3] * (0.4 + 0.3 * frandom()));
  glShadeModel(GL_SMOOTH);
