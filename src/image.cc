...
static void ImageGetRawData(Image *image, unsigned char *data) {
  ...
  for (i = 0; i < image->sizeY; i++) {
    for (k = 0; k < image->sizeZ; k++) ImageGetRow(image, image->tmp[k + 1], i, k);
    for (j = 0; j < image->sizeX; j++)
      for (k = 1; k <= image->sizeZ; k++) *data++ = *(image->tmp[k] + j);
    data += remain;
  }
}
...
void read_2d_image_rgb(char *image, unsigned char **rbuffer, int *tx, int *ty) {
  int w, h, c, pad = 0;
  ...
  *ty = base_image->sizeY;
  ...
  for (w = 0; w < *tx; w++) {
    for (h = 0; h < *ty; h++) {
      for (c = 0; c < 3; c++) {
        (*rbuffer)[(w + h * (*tx)) * 4 + c] =
            base_image->data[(w + base_image->sizeX * h) * base_image->sizeZ + pad + c];
      }
      if (base_image->sizeZ == 4)
        (*rbuffer)[(w + h * (*tx)) * 4 + 3] =
            base_image->data[(w + base_image->sizeX * h) * base_image->sizeZ + pad + 3];
      else
