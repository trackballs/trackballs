...
class Ball : public Animated {
 public:
  ...
  virtual Boolean crash(Real speed);             // Generic crash called when we hit ground etc. Returns if we are still alive
  virtual Boolean crash(Real speed,Ball *);      // When we crash into another ball. Returns if we are still alive
  ...
  int ballResolution;              /* 0 for low resolution balls (eg. debris), 1 for normal balls, 2 for high res balls (eg. player) */
