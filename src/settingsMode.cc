...
void SettingsMode::display() {
  ...
  if(testingResolution > 0.0) {
    snprintf(str,sizeof(str),_("Is resolution %dx%dx%d ok?"), screenWidth, screenHeight, colorDepth);
  ...
  case SUBSCREEN_GRAPHICS:
	...
	// Resolution
	snprintf(str,sizeof(str),"%dx%dx%d", screenResolutions[resolution][0], screenResolutions[resolution][1], colorDepth);
	...
	// Windowed
	menuItem_LeftRight(MENU_WINDOWED,menucount++,_("  Fullscreen"),(char*)(Settings::settings->is_windowed?_("No"):_("Yes")));

	// gfx details
	switch(Settings::settings->gfx_details) {
	case 0: snprintf(str,sizeof(str),_("None")); break;
	case 1: snprintf(str,sizeof(str),_("Minimalistic")); break;
	case 2: snprintf(str,sizeof(str),_("Simple")); break;
	case 3: snprintf(str,sizeof(str),_("Standard")); break;
	case 4: snprintf(str,sizeof(str),_("Extra")); break;
	case 5: snprintf(str,sizeof(str),_("Everything")); break;
	}
	menuItem_LeftRight(MENU_GFX_DETAILS,menucount++,_("  Details"),str);
	if(Settings::settings->doReflections)
	  menuItem_LeftRight(MENU_DO_REFLECTIONS,menucount++,_("  Reflections (beta)"),_("Yes"));
	else
	  menuItem_LeftRight(MENU_DO_REFLECTIONS,menucount++,_("  Reflections (beta)"),_("No"));

	// show FPS
	menuItem_LeftRight(MENU_SHOW_FPS,menucount++,_("  Show FPS"),(char*)(Settings::settings->showFPS?_("Yes"):_("No")));
  ...
  case SUBSCREEN_CONTROLS:	
	...
	/* Steering */
	if(Settings::settings->rotateSteering == 0) snprintf(str,sizeof(str),_("normal"));
	else if(Settings::settings->rotateSteering > 0) snprintf(str,sizeof(str),_("+%d degrees"),45*Settings::settings->rotateSteering);
	else if(Settings::settings->rotateSteering < 0) snprintf(str,sizeof(str),_("-%d degrees"),-45*Settings::settings->rotateSteering);
	...
	else if(SDL_NumJoysticks() == 0) snprintf(str,255,_("no joystick found"));
	else snprintf(str,255,"no joystick selected (%d found)",SDL_NumJoysticks());
  ...
  case SUBSCREEN_OTHER:
	...
	/* Language */
	menuItem_LeftRight(MENU_LANGUAGE,3,_("  Language"),Settings::languageNames[Settings::settings->language]);
...
void SettingsMode::mouseDown(int button,int x,int y) {
  ...
  case MENU_GFX_DETAILS:
	...
	break;
  case MENU_DO_REFLECTIONS: Settings::settings->doReflections = Settings::settings->doReflections ? 0 : 1; break;
  ...
  case MENU_SFX_VOLUME:
	...
	break;
  case MENU_LANGUAGE:
	settings->language = mymod(((int) (settings->language+(up?1:-1))),Settings::nLanguages);
	settings->setLocale();
