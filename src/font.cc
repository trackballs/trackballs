...
int char_corresp[256] = {
    ...
    '§',       '%',       '&',       '"',       '{',  '}',  '[',       ']',       '|',
    '`',       '\'',      '_',       '^',       '@',  '°',  '£',       '¤',       '\0',
    '\0',      256 + '"', 256 + 'C', 256 + 'R', '\0', '\0', 256 + '<', 256 + '>', 'Å',
    'Ä',       'Ö',       'å',       'ä',       'ö',  ' ',  '\0',      256 + '?', 256 + '*',
    256 + 'O', 256 + '/', 256 + 'o', 'P',       'R',  'S',  'T',       'k',       'l',
    'm',       'n',       256 + 'f', ' ',       '\0', '4',  '5',       '6',       '7',
    '-',       '+',       '<',       '>',       '#',  '~',  256 + '!', '\0',      256 + 'X',
    256 + 'V', 256 + 'l',
};
  ...
  for (jj = 0; jj < CHAR_HNB; jj++) {
    ...
    for (i = 0; i < CHAR_WNB; i++) {
      ...
      for (x = CHAR_W - 1; x >= 0; x--) {
        for (y = 0; y < CHAR_H; y++) {
          if ((((char *)pixels)[(i * CHAR_W + x +
                                 (j * CHAR_WNB * CHAR_W * CHAR_H + y * CHAR_WNB * CHAR_W)) *
                                    4 +
                                3]) != 0) { /* Not transparent. It is the letter */
            xs = x;
...
int Font::addText(int font, const char *text, int x, int y, float sx, float sy, double delay,
  ...
  if ((font < 0) || (font >= nb_fonts)) font = 0;
  return (current_fonts[font]->l_addText(text, x, y, sx, sy, delay, offset, stab, ampl, fix));
}
...
int Font::l_addText(const char *text, int x, int y, float sx, float sy, double delay,
  ...
  if (stab < BASE_TIME) stab = BASE_TIME;
  ...
  if (fix == 1) {
    for (j = 0; j < textlines[i].length; j++) { textlines[i].time[j] = -delay; }
    textlines[i].stable = 0;
    textlines[i].tmax = BASE_TIME;
  }

  if (fix == 2) {
    for (j = 0; j < textlines[i].length; j++) { textlines[i].time[j] = BASE_TIME; }
    textlines[i].stable = 1;
    textlines[i].tmax = BASE_TIME;
  }

  // update max_line
  if (i >= max_line_used) max_line_used = i + 1;
  return (1);
}
...
void Font::destroyAllText() {
  ...
  for (i = 0; i < max_line_used; i++) { destroyText(i); }
  max_line_used = 0;
}
...
void Font::destroyText(int tl) {
  ...
  if (textlines[tl].time != NULL) free(textlines[tl].time);
  ...
  if (tl == max_line_used - 1) {
    max_line_used--;
    if (max_line_used < 0) max_line_used = 0;
  }
}
...
void Font::l_destroyAllText() {
  ...
  for (i = 0; i < MAX_TEXT_LINE; i++) {
    if (textlines[i].text != NULL) { destroyText(i); }
  }
  max_line_used = 0;
}

/* find letter index */
static int get_letter_index_simple(int txt) { return (char_access[txt + 128]); }
...
void Font::draw_a_char(int index, int x, int y, float sx, float sy) {
  ...
  glTexCoord2f((float)i / (float)CHAR_WNB + (1. / CHARS_W), (float)j / (float)CHAR_HNB);
...
void Font::draw_a_text(int tl) {
  ...
  for (i = 0; i < textlines[tl].length; i++) {
    ...
    if (textlines[tl].time[i] < BASE_TIME) {
      ...
      draw_a_char(index, x, textlines[tl].y, textlines[tl].sizex * tm,
                  textlines[tl].sizey * tm);
    } else {
...
void Font::draw_highlight(int tl) {
  ...
  if ((tl < 0) || (textlines[tl].text == NULL)) return;
  ...
  draw_a_char(index, (int)(textlines[tl].x + w / 2. - 1.5 * textlines[tl].sizex),
              textlines[tl].y, w / 2., textlines[tl].sizey);
}
...
int Font::addCenterText(int font, const char *text, int x, int y, float sx, float sy,
                        double delay, double offset, double stab, double ampl, int fix) {
  if ((font < 0) || (font >= nb_fonts)) font = 0;
  return (current_fonts[font]->l_addCenterText(text, x, y, sx, sy, delay, offset, stab, ampl,
                                               fix));
}

// same than 'add_text' but 'x' is the X-center of the text to draw
int Font::l_addCenterText(const char *text, int x, int y, float sx, float sy, double delay,
                          double offset, double stab, double ampl, int fix) {
  int width;
...
int Font::addRightText(int font, const char *text, int x, int y, float sx, float sy,
                       double delay, double offset, double stab, double ampl, int fix) {
  if ((font < 0) || (font >= nb_fonts)) font = 0;
  return (
      current_fonts[font]->l_addRightText(text, x, y, sx, sy, delay, offset, stab, ampl, fix));
}

// same than 'add_text' but 'x' is the right-cborder of the text to draw
int Font::l_addRightText(const char *text, int x, int y, float sx, float sy, double delay,
                         double offset, double stab, double ampl, int fix) {
  int width;
...
int Font::isTextDrawn() {
  ...
  for (i = 0; i <= max_line_used; i++) {
    if (textlines[i].text != NULL) { return (1); }
  }
  return (0);
}
...
int Font::getSelectedText(int mx, int my) {
  ...
  for (i = 0; i <= max_line_used; i++) {
    if (textlines[i].text != NULL) {
      if ((mx >= textlines[i].x0) && (mx <= textlines[i].x1) && (my >= textlines[i].y0) &&
          (my <= textlines[i].y1))
        return (i);
    }
  }
...
void Font::draw() {
  ...
  for (i = 0; i <= max_line_used; i++) {
    if (textlines[i].text != NULL) {
      ...
      glBegin(GL_QUADS);
      if (textlines[i].pulse) { textlines[i].font->draw_highlight(i); }
    ...
    if (textlines[i].text != NULL) {
      if (textlines[i].removing) {
        ...
        for (j = 0; j < textlines[i].length; j++) {
          ...
          if (textlines[i].time[j] > 0) stable = 0;
        }
        if (stable) destroyText(i);
        continue;
      }
      if (textlines[i].pulse) textlines[i].time_pulse += t;
      if ((textlines[i].time_pulse > 0.) && (!textlines[i].pulse))
        textlines[i].time_pulse -= t;
      if ((textlines[i].stable) && (!textlines[i].pulse)) continue;
      stable = 1;
      for (j = 0; j < textlines[i].length; j++) {
        textlines[i].time[j] += t;
        if (textlines[i].time[j] < textlines[i].tmax) stable = 0;
      }
      if (stable) textlines[i].stable = 1;
    }
  }
}
...
void Font::setPulseText(int tl, int pulse) {
  ...
  if (textlines[tl].pulse == pulse) return;
...
int Font::drawSimpleText(int font, const char *text, int x, int y, float sx, float sy, float r,
                         float g, float b, float a) {
  if ((font < 0) || (font >= nb_fonts)) font = 0;
  return (current_fonts[font]->l_drawSimpleText(text, x, y, sx, sy, r, g, b, a));
}

int Font::drawRightSimpleText(int font, const char *text, int x, int y, float sx, float sy,
                              float r, float g, float b, float a) {
  if ((font < 0) || (font >= nb_fonts)) font = 0;
  return (current_fonts[font]->l_drawRightSimpleText(text, x, y, sx, sy, r, g, b, a));
}

int Font::drawCenterSimpleText(int font, const char *text, int x, int y, float sx, float sy,
                               float r, float g, float b, float a) {
  if ((font < 0) || (font >= nb_fonts)) font = 0;
...
int Font::l_drawRightSimpleText(const char *text, int x, int y, float sx, float sy, float r,
                                float g, float b, float a) {
  return (l_drawSimpleText(text, x - l_getTextWidth(text, sx), y, sx, sy, r, g, b, a));
}

int Font::l_drawCenterSimpleText(const char *text, int x, int y, float sx, float sy, float r,
                                 float g, float b, float a) {
